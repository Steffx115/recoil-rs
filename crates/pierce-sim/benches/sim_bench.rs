//! Criterion benchmarks for pierce-sim hot paths.
//!
//! Run with: `cargo bench -p pierce-sim`

use criterion::{black_box, criterion_group, criterion_main, Criterion};

use bevy_ecs::prelude::*;
use pierce_sim::components::{
    Allegiance, CollisionRadius, Heading, Health, MoveState, MovementParams, Position, UnitType,
    Velocity,
};
use pierce_sim::lifecycle::spawn_unit;
use pierce_sim::sim_runner::{init_sim_world, sim_tick};
use pierce_sim::spatial::SpatialGrid;
use pierce_sim::{SimFloat, SimVec2, SimVec3};
...
fn rebuild_grid(world: &mut World) {
    let entities: Vec<(Entity, SimVec2)> = world
        .query_filtered::<(Entity, &Position), Without<pierce_sim::Dead>>()
        .iter(world)
...
fn bench_collision_2000(c: &mut Criterion) {
    ...
    c.bench_function("collision_2000", |b| {
        b.iter(|| {
            ...
            rebuild_grid(&mut world);
            pierce_sim::collision::collision_system(black_box(&mut world));
        });
    });
}
...
fn bench_movement_2000(c: &mut Criterion) {
    ...
    c.bench_function("movement_2000", |b| {
        b.iter(|| {
            pierce_sim::movement::movement_system(black_box(&mut world));
        });
    });
}
